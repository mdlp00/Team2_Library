...
//-----그룹페이지처리 이전 --------------------------------------------------------------------------------------------		
		if(isPrevPage){
			sb.append("<a href ='#'  onclick='search_book("+  (startPage - 1)   +")'");
			sb.append("'>◀</a>");
		}
		else
			sb.append("◀");
...
//------페이지 목록 출력 -------------------------------------------------------------------------------------------------
		...
		for(int i=startPage; i<= endPage ;i++){
			...
			if(i == nowPage){ //현재 있는 페이지
				...
				sb.append("<a href ='#' onclick='search_book("+  (i)   +")'>");
				sb.append(i);
				sb.append("</a>");
...
//-----그룹페이지처리 다음 ----------------------------------------------------------------------------------------------
		if(isNextPage){
			...
			sb.append("<a href ='#' onclick='search_book("+  (endPage+1)   +")'");
			sb.append("'>▶</a>");
		}
		else
			sb.append("▶");
